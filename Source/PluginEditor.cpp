	...
	: AudioProcessorEditor(&p), audioProcessor(p)
{
	...
	fileBrowserIR1.onFileDoubleClick = [this](juce::File file) {
		printf("Left adress After: %d", (int)&audioProcessor.procLeft);
		audioProcessor.procLeft.setFile(file, &audioProcessor.valueTree);
		};

	fileBrowserIR2.onFileDoubleClick = [this](juce::File file)
		{audioProcessor.procRight.setFile(file, &audioProcessor.valueTree); };
	...
	buttonLoadReverb.onClick = [this]() {
		...
		std::function<void(const juce::FileChooser&)> fileCallback = [this](const juce::FileChooser& _fileChooser) {
			...
			audioProcessor.procOut.setFile(file, &audioProcessor.valueTree);
			buttonLoadReverb.setButtonText(juce::String("Reverb: ") + audioProcessor.procOut.file.getFileName());
			};
